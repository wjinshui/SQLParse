...
public class DrawTree extends JPanel {  
    ...
    private SQLTreeNode tree;              //保存整棵树  
     ...
     */  
    public DrawTree(int childAlign){  
     ...
     * @param childAlign 对齐策略 
     */  
    public DrawTree(SQLTreeNode n, int childAlign){  
        super();  
        setTree(n);  
     ...
     * @param n 用于绘制的树的 
     */  
    public void setTree(SQLTreeNode n) {  
        tree = n;        
    }  
     ...
     * @param g 绘图上下文环境 
     */  
    public void drawAllNode(SQLTreeNode n, int x, Graphics g){  
        int y =3 *(vGap+gridHeight)+startY;  
        ...
        if(n.hasChild()){  
            List<SQLTreeNode> c = n.getChildren();  
